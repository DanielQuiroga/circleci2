...
make_task_def(){
	task_template='[
		{
			"name": "node-sample-webapp",
			"image": "%s.dkr.ecr.us-east-1.amazonaws.com/node-sample-webapp:%s",
			"essential": true,
			"memory": 200,
			"cpu": 10,
			"portMappings": [
...
push_ecr_image(){
	...
	docker push $AWS_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/node-sample-webapp:$CIRCLE_SHA1
}
